...
class MultiLayerNN:
    ...
    def Start(self):
        ...
        self.Read("/home/abdlrhman/Desktop/NeuralNetworksProject-SemiFinalDelivery/Training/", "/home/abdlrhman/Desktop/NeuralNetworksProject-SemiFinalDelivery/TestFiles/")
    ...
    def DisplayImages(self):
        ...
        for each in glob(self.test_path + "*"):
            if ".png" in each:
             ...
             self.OriginalImagesCB.addItem(each.split("/")[-1])
            count += 1
    ...
    def on_click(self):
        ...
        if self.Classifier.currentText() == "MLP":
            ...
            for j in range(len(pos)):  # iterate over objects
                ...
                label = str(labels[j])
                cv.putText(im, label, (pos[j][0], pos[j][2]+15), font, 0.7, (0, 255, 0), 2, cv.LINE_AA)
            cv.imshow("test", im)
        else:
